    ...
    for vehicle in company.vehicles:
        with dpg.table_row(parent="vehicles_table"):
            ...
            if isinstance(vehicle, Ship):
                ...
                dpg.add_text(f"Цвет грузовика: {vehicle.color}")
            else:
...
def show_vehicle_form():
    ...
    with dpg.window(label="Добавить транспорт", width=400, height=300, modal=True, tag="vehicle_form"):
        ...
        dpg.add_combo(["Судно", "Грузовик"], tag="vehicle_type", width=250, callback=toggle_vehicle_specific_fields)
        ...
        dpg.add_text("Введите цвет грузовика:", tag="color_truck", show=False)
...
def export_results():
    ...
    dpg.set_value("status", "Результаты экспортированы в файл dump.json.")
...
def show_about():
    ...
    with dpg.window(label="О программе", width=400, height=200, modal=True, tag="about_window"):
        dpg.add_text("Программа: Компания")
        dpg.add_text("Лабораторная работа номер 12")
        dpg.add_text("Вариант: 2")
        dpg.add_text("Разработчик: Бабицкий Алексей ")
        dpg.add_text("Группа 82 ТП")
        dpg.add_text("Описание: Клиенты и транспортные средства")
